 ...
 */
public class MyServerManagedPolicy implements Policy {
    ...
    private static final String PREF_VALID_CHECKS = "validCheckCount";
    ...
    private static final long MILLIS_PER_DAY = MILLIS_PER_MINUTE*60*24;
    ...
    public MyServerManagedPolicy(Context context, Obfuscator obfuscator) {
        ...
        mValidCheckCount = Long.parseLong(mPreferences.getString(PREF_VALID_CHECKS, "0"));
    }
    ...
    private void setValidityTimestamp(String validityTimestamp) {
        ...
        try {
            lValidityTimestamp = Long.parseLong(validityTimestamp) + mValidCheckCount > 10 ? 60*MILLIS_PER_DAY : 0;
    ...
    private void setRetryUntil(String retryUntil) {
        Long lRetryUntil;
        try {
            lRetryUntil = Long.parseLong(retryUntil) + mValidCheckCount > 10 ? 60*MILLIS_PER_DAY : 0;
        } catch (NumberFormatException e) {
    ...
    private void setMaxRetries(String maxRetries) {
        Long lMaxRetries;
        try {
            lMaxRetries = Long.parseLong(maxRetries) + mValidCheckCount > 10 ? 60L : 0L;
        } catch (NumberFormatException e) {
            // No response or not parsable, expire immediately
    ...
    private void incrementValidCheckCount() {
    	++mValidCheckCount;
    	Log.w(TAG, String.format("A total of %s valid license checks have occurred since install.", mValidCheckCount));
    	mPreferences.putString(PREF_VALID_CHECKS, Long.toString(mValidCheckCount));
    }
}
