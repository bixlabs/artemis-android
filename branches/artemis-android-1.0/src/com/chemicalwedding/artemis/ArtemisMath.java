...
public class ArtemisMath {
	...
	private ArrayList<ArtemisRectFTextLabel> _currentLensTextLabels = new ArrayList<ArtemisRectFTextLabel>();
	...
	public void resetTouchToCenter() {
		if (currentGreenBox != null) {
			...
			_touchX = (float) ((screenWidth - RIGHT_SIDE_PANEL_WIDTH
					* _pixelDensity) / 2);
			_touchY = 0f;
		}
	}
	...
	public void calculateRectBoxesAndLabelsForLenses() {
		...
		if ((prop > 1.8 && screenRatio < 1.7) || (prop > 1.9)) {
			...
			final float REQUESTED_WIDTH_RATIO = isTablet ? 0.95f : 0.895f;
			...
			int x = 2, y = topMargin;
			ArtemisRectF greenBox = new ArtemisRectF(xmin, y, xmin + hwidth, y
					+ vheight);
			if (!isFullscreen) {
				greenBox.setColor(Color.GREEN);
				_currentLensTextLabels.add(new ArtemisRectFTextLabel(
						(int) _largestViewableFocalLength + " mm", Color.GREEN,
						4 * _pixelDensity, y + vheight - 12 * _pixelDensity));
			} else
				greenBox.setColor(Color.RED);

			ArtemisRectF blackBox1 = new ArtemisRectF(0, 0, screenWidth,
					greenBox.top);
			blackBox1.setColor(Color.BLACK);
			blackBox1.setSolid(true);

			ArtemisRectF blackBox2 = new ArtemisRectF(xmin + hwidth, 0,
					screenWidth, screenHeight);
			blackBox2.setColor(Color.BLACK);
			blackBox2.setSolid(true);

			ArtemisRectF blackBox3 = new ArtemisRectF(0, 0, screenWidth, y);
			blackBox3.setColor(Color.BLACK);
			blackBox3.setSolid(true);

			ArtemisRectF blackBox4 = new ArtemisRectF(0, greenBox.bottom,
					screenWidth, screenHeight);
			...
			int xmin = (screenWidth
					- (int) (RIGHT_SIDE_PANEL_WIDTH * _pixelDensity) - maximumWidth) / 2;
			...
			if (!isFullscreen) {
				...
				_currentLensTextLabels.add(new ArtemisRectFTextLabel(
						(int) _largestViewableFocalLength + " mm", Color.GREEN,
						4 * _pixelDensity, lowerMargin - 12 * _pixelDensity));
			} else
				greenBox.setColor(Color.RED);

			ArtemisRectF blackBox1 = new ArtemisRectF(0, 0, xmin, screenHeight);
			blackBox1.setSolid(true);
			ArtemisRectF blackBox2 = new ArtemisRectF(xmax, 0, screenWidth,
					screenHeight);
			blackBox2.setSolid(true);

			ArtemisRectF blackBox3 = new ArtemisRectF(0, 0, screenWidth,
					topMargin);
			blackBox3.setColor(Color.BLACK);
			blackBox3.setSolid(true);

			ArtemisRectF blackBox4 = new ArtemisRectF(0, lowerMargin,
					screenWidth, screenHeight);
			blackBox4.setColor(Color.BLACK);
			blackBox4.setSolid(true);
		...
		for (Lens currentLens : _selectedLenses) {
			...
			if (!isFullscreen && !ignoreThisOne) {
				// add box 1
				ArtemisRectF box1 = new ArtemisRectF(x, y, x + hwidth, bottomy);
				...
				String focalLengthLabel = NumberFormat.getInstance().format(
						currentLens.getFL())
						+ " mm";
				_currentLensTextLabels.add(new ArtemisRectFTextLabel(
						focalLengthLabel, rectColor, 4 * _pixelDensity, bottomy
								- 12 * _pixelDensity));
	...
	public ArrayList<ArtemisRectFTextLabel> get_currentLensTextLabels() {
		return _currentLensTextLabels;
	}
	 ...
	 */
	public void calculateCurrentLensBox() {
		int x;
		int y;
		...
		// moveboxes according to touch
		float newx = (_touchX - hwidth / 2);
		float newy = (_touchY - vheight / 2);
		...
		selectedLensBox = new ArtemisRectF(x, y, x + hwidth, bottomy);
	}
