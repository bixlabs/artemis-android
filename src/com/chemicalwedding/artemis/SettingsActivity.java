		...
		public void onCreate(Bundle savedInstanceState) {
			...
			if (CameraPreview14.supportedExposureLevels != null) {
				...
				for (int i = 0; i < exposureLabels.length; i++) {
					exposureLabels[i] = CameraPreview14.supportedExposureLevels
							.get(i).toString();
				}
				exposurePref.setEntries(exposureLabels);
				exposurePref.setEntryValues(exposureLabels);

				int exposureIndex = getSharedPreferences()
						.getInt(getString(R.string.preference_key_selectedexposurelevel),
								-1);
				if (exposureIndex == -1) {
					exposureIndex = CameraPreview14.supportedExposureLevels
							.size();
					if (exposureIndex > 0) {
						exposureIndex = (exposureIndex / 2) - 1;
					}
				...
				exposurePref
						.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
							...
							public boolean onPreferenceChange(
								...
								int index = 0;
								for (Integer val : CameraPreview14.supportedExposureLevels) {
									if (val.equals(newValueInt)) {
										getSharedPreferences()
												.edit()
